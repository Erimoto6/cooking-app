...
<div class="cv-screen">
  ...
  <p class="cv-count">{{ recipes|length }} recipes available</p>

  <div class="cv-filters">
    <button class="cv-filter-btn active" onclick="filterRecipes('all')">All</button>
    <button class="cv-filter-btn" onclick="filterRecipes('Main Course')">Main</button>
    <button class="cv-filter-btn" onclick="filterRecipes('Dessert')">Desserts</button>
    <button class="cv-filter-btn" onclick="filterRecipes('Beverage')">Drinks</button>
  </div>

  {% for region, region_recipes in recipes_by_region.items() %}
  <div class="cv-region-block">
    <p class="cv-region-label">{{ region }}</p>
    <div class="cv-grid">
      {% for recipe in region_recipes %}
      <a href="{{ url_for('view_recipe_by_id', recipe_id=recipe.id) }}" class="cv-card" data-category="{{ recipe.category }}">
        <div class="cv-card-title">{{ recipe.title }}</div>
        <div class="cv-card-meta">{{ recipe.category or 'Main Course' }}</div>
      </a>
      {% endfor %}
    </div>
  </div>
...
<script>
  function filterRecipes(category) {
    document.querySelectorAll('.cv-filter-btn').forEach(btn => btn.classList.remove('active'));
...
{% endblock %}'''
    ...
    return "✅ cuisine_view.html template created! Now visit a cuisine page."
